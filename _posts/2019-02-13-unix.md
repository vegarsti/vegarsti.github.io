...
```
$> ls dataset-directory | grep '\d\d\d\d_A.csv' | cut -c 1-4 | python3 parse.py | uniq
1
2
3
5
...
```
$> comm <(ls dataset-directory | grep '\d\d\d\d_A.csv' | cut -c 1-4 | python3 parse.py | uniq) <(seq 500)
        1
        2
        3
    4
...
```
$> comm -1 -3 <(ls dataset-directory | grep '\d\d\d\d_A.csv' | cut -c 1-4 | python3 parse.py | uniq) <(seq 500)
4
8
...
500
